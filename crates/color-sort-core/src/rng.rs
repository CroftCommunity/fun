...
//! primitive identical to the other cores.

use rand::{Rng, SeedableRng};
use rand_chacha::ChaCha20Rng;

/// Seeded deterministic RNG. `draws` counts the index draws consumed.
#[derive(Clone)]
pub struct DetRng {
    inner: ChaCha20Rng,
    draws: u64,
...
impl DetRng {
    ...
    /// In-place Fisher–Yates shuffle, top index down: for `i` from `len-1` down
    /// to `1`, swap `slice[i]` with `slice[index(i+1)]`.

...
//! `(packed_seed, moves)` pair — the packed seed regenerates the exact board —
//! and re-derive the final [`state_hash`] and whether the board cleared. A clean
//! solve is `Won` with no declared assistance; mistakes and hints are UI-side
//! (not part of the move list), graded for display by [`crate::score`].

use crate::board::Board;
use crate::config::{daily_config, level_config, Config};
...
impl Game {
    /// Start a fresh game for a given board config.
    #[must_use]
    ...
    /// The packed seed that a record carries — replay regenerates the exact
    /// board from it (see [`Config::to_packed`]).
    #[must_use]
    pub fn packed_seed(&self) -> u64 {
        self.config.to_packed()
    }
}
...
impl pond_outcome::Game for LooseEnds {
    ...
    fn replay(packed_seed: u64, moves: &[u32]) -> pond_outcome::Replayed {
        let mut board = generate(&Config::from_packed(packed_seed));
        for &id in moves {

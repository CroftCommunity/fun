//! Trio Tumble's deterministic match-3 engine: match / clear / gravity / refill / cascade.
...
/// `Game::play_move` calls this after every move, so `TrioTumble::replay` reshuffles
...
/// `TrioTumble::VERSION` bump. Cost is a build-time/analysis concern, not the
/// runtime path.
#[must_use]

...
//! the game RNG) into a live, match-free board — so `TrioTumble::replay` reshuffles
//! identically and the outcome stays verifiable. These tests pin the pure
//! reshuffle op and the post-move invariant it upholds.

use trio_tumble_core::rng::DetRng;
use trio_tumble_core::{deal, find_matches, has_legal_move, reshuffle_if_dead, Board, Game};
...
fn play_move_never_leaves_a_reachable_deadlock() {
    ...
    for seed in 0..40u64 {
        ...
        for _ in 0..20 {
            let swaps = trio_tumble_core::legal_swaps(&game.board);

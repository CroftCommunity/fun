//! Browser binding over [`trio_tumble_core`] for the games shelf — raw C-ABI +
...
use trio_tumble_core::{
    ...
    ChecklistProgress, ChecklistTargets, Game as TtGame, Obstacle, SpecialKind,
};

// --- level config (see plans/2026-07-30-trio-tumble-playable.md) ---
const WIDTH: usize = 8;
...
static PAR_TABLE_JSON: &[u8] = include_bytes!("../../../games/trio-tumble/par-pack.json");
...
pub extern "C" fn new_game(seed_lo: u32, seed_hi: u32) {
    ...
    set_session(Session {
        ...
        game: TtGame::new(board, seed, COLORS),
        budget: MOVE_BUDGET,
...
pub extern "C" fn new_blockers_game(seed_lo: u32, seed_hi: u32) {
    ...
    set_session(Session {
        seed,
        mode: Mode::Blockers,
        game: TtGame::new(board, seed, blockers_mode::COLORS),
...
pub extern "C" fn new_jelly_game(seed_lo: u32, seed_hi: u32) {
    ...
    set_session(Session {
        seed,
        mode: Mode::Jelly,
        game: TtGame::new(board, seed, jelly_mode::COLORS),
...
pub extern "C" fn new_ingredients_game(seed_lo: u32, seed_hi: u32) {
    ...
    set_session(Session {
        ...
        game: TtGame::new(board, seed, ingredients_mode::COLORS),
...
pub extern "C" fn new_checklist_game(seed_lo: u32, seed_hi: u32) {
    ...
    set_session(Session {
        seed,
        mode: Mode::Checklist,
        game: TtGame::new(board, seed, checklist_mode::COLORS),
...
pub extern "C" fn new_obstacles_game(seed_lo: u32, seed_hi: u32) {
    ...
    set_session(Session {
        seed,
        mode: Mode::Obstacles,
        game: TtGame::new(board, seed, obstacles_mode::COLORS),
...
pub extern "C" fn play_swap_traced(r1: u32, c1: u32, r2: u32, c2: u32) -> *const u8 {
    ...
    s.swaps.push([r1 as u8, c1 as u8, r2 as u8, c2 as u8]);
    let frames: Vec<Vec<String>> = snapshots
        .iter()
        .map(trio_tumble_core::Board::to_rows)
        .collect();
...
/// The `pond-outcome` [`Game`] impl for Trio Tumble — replay `(seed, swaps)` by
/// dealing the board and applying the swaps, grading the final score.
struct TrioTumble;
impl Game for TrioTumble {
    type Move = [u8; 4];
    const KIND: &'static str = "trio-tumble";
    const VERSION: u32 = 1;
    fn replay(seed: u64, moves: &[[u8; 4]]) -> Replayed {
        let board = deal(seed, WIDTH, HEIGHT, COLORS);
        let mut game = TtGame::new(board, seed, COLORS);
        for m in moves {
...
struct TrioTumbleBlockers;
impl Game for TrioTumbleBlockers {
    type Move = [u8; 4];
    const KIND: &'static str = "trio-tumble-blockers";
    const VERSION: u32 = 1;
    fn replay(seed: u64, moves: &[[u8; 4]]) -> Replayed {
        let board = deal_blockers(
            seed,
        ...
        let mut game = TtGame::new(board, seed, blockers_mode::COLORS);
        for m in moves {
            let _ = game.play_move(
                (m[0] as usize, m[1] as usize),
                (m[2] as usize, m[3] as usize),
...
struct TrioTumbleJelly;
impl Game for TrioTumbleJelly {
    type Move = [u8; 4];
    const KIND: &'static str = "trio-tumble-jelly";
    const VERSION: u32 = 1;
    fn replay(seed: u64, moves: &[[u8; 4]]) -> Replayed {
        let board = deal_jelly(
            seed,
        ...
        let mut game = TtGame::new(board, seed, jelly_mode::COLORS);
        for m in moves {
            let _ = game.play_move(
                (m[0] as usize, m[1] as usize),
                (m[2] as usize, m[3] as usize),
...
/// no score/stars.
struct TrioTumbleIngredients;
impl Game for TrioTumbleIngredients {
    type Move = [u8; 4];
    const KIND: &'static str = "trio-tumble-ingredients";
    const VERSION: u32 = 1;
    fn replay(seed: u64, moves: &[[u8; 4]]) -> Replayed {
        let board = deal_ingredients(
            seed,
        ...
        let mut game = TtGame::new(board, seed, ingredients_mode::COLORS);
        for m in moves {
            let _ = game.play_move(
                (m[0] as usize, m[1] as usize),
                (m[2] as usize, m[3] as usize),
...
/// score/stars.
struct TrioTumbleChecklist;
impl Game for TrioTumbleChecklist {
    type Move = [u8; 4];
    const KIND: &'static str = "trio-tumble-checklist";
    const VERSION: u32 = 1;
    fn replay(seed: u64, moves: &[[u8; 4]]) -> Replayed {
        let board = deal(
            seed,
        ...
        let mut game = TtGame::new(board, seed, checklist_mode::COLORS);
...
struct TrioTumbleObstacles;
impl Game for TrioTumbleObstacles {
    type Move = [u8; 4];
    const KIND: &'static str = "trio-tumble-obstacles";
    const VERSION: u32 = 1;
    fn replay(seed: u64, moves: &[[u8; 4]]) -> Replayed {
        let board = deal_obstacles(
            seed,
        ...
        let mut game = TtGame::new(board, seed, obstacles_mode::COLORS);
        for m in moves {
            let _ = game.play_move(
                (m[0] as usize, m[1] as usize),
                (m[2] as usize, m[3] as usize),
...
/// envelope `kind` distinguishes the modes (`trio-tumble` / `trio-tumble-blockers` /
/// `trio-tumble-jelly` / `trio-tumble-ingredients` / `trio-tumble-checklist` / `trio-tumble-obstacles`).
#[no_mangle]
pub extern "C" fn outcome_json(declare: u32) -> *const u8 {
    ...
    let bytes = match s.mode {
        Mode::TargetScore => {
            let record = attest::<TrioTumble>(s.seed, s.swaps.clone(), Outcome::Lost, assistance);
            pond_outcome::to_doc::<TrioTumble>(&record)
        }
        Mode::Blockers => {
            let record =
                attest::<TrioTumbleBlockers>(s.seed, s.swaps.clone(), Outcome::Lost, assistance);
            pond_outcome::to_doc::<TrioTumbleBlockers>(&record)
        }
        Mode::Jelly => {
            let record =
                attest::<TrioTumbleJelly>(s.seed, s.swaps.clone(), Outcome::Lost, assistance);
            pond_outcome::to_doc::<TrioTumbleJelly>(&record)
        }
        Mode::Ingredients => {
            let record =
                attest::<TrioTumbleIngredients>(s.seed, s.swaps.clone(), Outcome::Lost, assistance);
            pond_outcome::to_doc::<TrioTumbleIngredients>(&record)
        }
        Mode::Checklist => {
            let record =
                attest::<TrioTumbleChecklist>(s.seed, s.swaps.clone(), Outcome::Lost, assistance);
            pond_outcome::to_doc::<TrioTumbleChecklist>(&record)
        }
        Mode::Obstacles => {
            let record =
                attest::<TrioTumbleObstacles>(s.seed, s.swaps.clone(), Outcome::Lost, assistance);
            pond_outcome::to_doc::<TrioTumbleObstacles>(&record)
        }
    };
    match bytes {
        Ok(bytes) => set_out(bytes),

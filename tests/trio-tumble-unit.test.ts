//! Unit tests for Trio Tumble's pure logic: the share round-trip, verify-
//! orchestration against the REAL wasm (+ tamper), and the result screen.

import { readFile } from "node:fs/promises";
import { beforeAll, describe, expect, it } from "vitest";

import { decodeRecord, encodeRecord, verifyRecord, type M3Envelope } from "../src/games/trio-tumble-outcome.js";
import { renderResultScreen } from "../src/games/trio-tumble.js";
import { TrioTumble, type Swap } from "../src/games/trio-tumble-wasm.js";
import { analyzeCascade } from "../src/games/trio-tumble-fx.js";

const WASM = "target/wasm32-unknown-unknown/release/trio_tumble_wasm.wasm";

async function loadReal(): Promise<TrioTumble> {
  const bytes = await readFile(WASM);
  const orig = globalThis.fetch;
  globalThis.fetch = (async () =>
    new Response(bytes, { headers: { "content-type": "application/wasm" } })) as typeof fetch;
  try {
    return await TrioTumble.load();
  } finally {
...
describe("share encode/decode", () => {
  it("round-trips a Trio Tumble envelope through deflated base64url", async () => {
...
describe("clear-the-blockers verify-orchestration (real wasm)", () => {
  ...
  it("grades a cleared blocker board as a verifiable Won", () => {
    expect(env.kind).toBe("trio-tumble-blockers");
...
describe("clear-the-jelly verify-orchestration (real wasm)", () => {
  ...
  it("grades a scrubbed jelly board as a verifiable Won", () => {
    expect(env.kind).toBe("trio-tumble-jelly");
    expect(env.payload.result).toBe("Won");
...
describe("checklist verify-orchestration (real wasm)", () => {
  ...
  it("grades a completed checklist as a verifiable Won", () => {
    expect(env.kind).toBe("trio-tumble-checklist");
    expect(env.payload.result).toBe("Won");
...
describe("clear-the-obstacles verify-orchestration (real wasm)", () => {
  ...
  it("grades a cleared obstacle board as a verifiable Won", () => {
    expect(env.kind).toBe("trio-tumble-obstacles");
    expect(env.payload.result).toBe("Won");
...
describe("target-score par table (real wasm)", () => {
  it("serves the baked ladder tiers for a daily (in-table) seed", async () => {
    ...
    const pack = JSON.parse(await readFile("games/trio-tumble/par-pack.json", "utf8")) as {

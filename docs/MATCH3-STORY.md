...
Narrative direction for `/match3/`. **Phase 2 ships the scaffold** (a game-event
...
(`src/games/match3-events.ts`). `src/games/match3-story.ts` maps a subset of those
...
pop-ups. The overlay (`src/games/match3-overlay.ts`) renders the beat as a

...
RUSTC="$(rustup which --toolchain stable rustc)" "$(rustup which --toolchain stable cargo)" build \
  --manifest-path "$fun/Cargo.toml" -p solitaire-wasm -p match3-wasm -p bubble-wasm -p wyrdle-wasm -p twenty48-wasm -p drop4-wasm -p align-wasm -p blockdoku-wasm -p looseends-wasm -p color-sort-wasm --release \

...
| `d-game-frame.html` | **Direction D — The Game Frame** (2026-08-30). Not a look: the structure of every *game page* — five fixed-height bands, one control vocabulary (verbs / setup / preferences / meters), a start screen, a dock on phones that becomes a rail on desktop. Mocked on Othello, Solitaire and Trio Tumble, with the measured diagnosis (the 110px wrapped header, the named board-jump causes) and the open questions. **Shipped 2026-08-30** (`plans/2026-08-30-plan-game-frame.md`, Phases 0–22): every game is on the frame; what shipped differs from the mock in two places — the setup card lives in a sheet behind "New game" rather than on the poster alone, and the rail's inline settings replace the desktop's settings sheet |
| `e-color-sort.html` | **Mock E — Color Sort** (2026-08-29, v1–v4). Endless-first poster with the locked Daily, the pour in three skins (animated), settings with Pour speed, the sign-in offer and sheet, desktop. The first mock with a **parity contract**: `e-color-sort.claims.json` names a spec per promise and `tests/mock-parity.test.ts` gates the plan's phases on them; `snaps/` holds captures from `tools/mock-snaps.mjs`, Before and Shipped |
| `f-play-surface.html` | **Mock F — The play surface** (2026-09-04, v1). What happens inside the frame's stage on every game page: the poster's Play reachable (built), a board that fills the stage (built, eight grids + Color Sort + Orchard), a toast lane (built), and seven **sketches** for choosing a direction — a ground per game, one shape for hand controls (2048's pad optional, Align split under two thumbs), Furrow upright, beats on the versus boards, Bubble as emoji, chess with presence, Loose Ends' curve. Captures live in `snaps/f-play-surface/` as `current.*` (main) beside `proposed.*` (the branch) — the first mock to carry both from the tool; `tools/mock-snaps.mjs all --out <slug> --tag <word>` |
| `g-looseends-mechanic.html` | **Mock G — Loose Ends, the knot has a key** (2026-09-08, v1–v2). The research pass mock F's Q10 parked, drawn once and then built: **locks** (an arrow that stays put until the arrow tied to it has gone) as the second mechanic from level 8, colour bands parked with the reason, the par already the star rule. Five decisions taken at their recommendations; shipped in four phases the same day (#89, #90, #91); v2 carries **Shipped** captures of level 8 beside the sketch, taken on the hermetic `?play=1&level=8` route. Plan `plans/2026-09-08-plan-looseends-mechanic.md` (DONE). Captures in `snaps/g-looseends-mechanic/` |

...
else missingArtifact("note: solitaire.wasm not built yet — run `npm run build:wasm` (Phase D needs it)");
...
else missingArtifact("note: trio-tumble.wasm not built yet — run `npm run build:wasm` (Trio Tumble needs it)");
...
else missingArtifact("note: bubble.wasm not built yet — run `npm run build:wasm` (bubble needs it)");
...
else missingArtifact("note: orchard-drop.wasm not built yet — run `npm run build:wasm`");
...
else missingArtifact("note: wyrdle.wasm not built yet — run `npm run build:wasm` (wyrdle needs it)");
...
else missingArtifact("note: 2048.wasm not built yet — run `npm run build:wasm` (2048 needs it)");
...
else missingArtifact("note: drop4.wasm not built yet — run `npm run build:wasm` (drop4 needs it)");
...
else missingArtifact("note: othello.wasm not built yet — run `npm run build:wasm` (othello needs it)");
...
else missingArtifact("note: dots.wasm not built yet — run `npm run build:wasm` (dots needs it)");
...
else missingArtifact("note: furrow.wasm not built yet — run `npm run build:wasm` (furrow needs it)");
...
else missingArtifact("note: cribbage.wasm not built yet — run `npm run build:wasm` (cribbage needs it)");
...
else missingArtifact("note: align.wasm not built yet — run `npm run build:wasm` (align needs it)");
...
else missingArtifact("note: blockdoku.wasm not built yet — run `npm run build:wasm` (blockdoku needs it)");
...
else missingArtifact("note: daily-pack.json missing — solitaire's daily mode needs it (Phase S)");
...
else missingArtifact("note: trio-tumble-blockers-pack.json missing — Trio Tumble's clear-the-blockers daily needs it");
...
else missingArtifact("note: trio-tumble-jelly-pack.json missing — Trio Tumble's clear-the-jelly daily needs it");
...
else missingArtifact("note: trio-tumble-ingredients-pack.json missing — Trio Tumble's ingredients daily needs it");
...
else missingArtifact("note: trio-tumble-checklist-pack.json missing — Trio Tumble's checklist (orders) daily needs it");
...
else missingArtifact("note: trio-tumble-obstacles-pack.json missing — Trio Tumble's clear-the-obstacles daily needs it");
...
else missingArtifact("note: trio-tumble-campaign-pack.json missing — Trio Tumble's campaign needs it");
...
else missingArtifact("note: bubble-daily-pack.json missing — bubble's daily mode needs it (B4)");
...
else missingArtifact("note: wyrdle-daily-pack.json missing — wyrdle's daily mode needs it (W3)");
...
else missingArtifact("note: 2048-daily-pack.json missing — 2048's daily mode needs it (T3)");
...
else missingArtifact("note: blockdoku-daily-pack.json missing — blockdoku's daily mode needs it (B9)");
...
/**
 * A built artifact (a wasm module, a daily pack) is missing. Locally that is a note — a developer working on one
 * game need not build the others. Under CI it is an error: the e2e shards download
 * the modules from the `wasm` job, and a shard that lost its artifact would
 * otherwise serve a shelf with no engines and pass whatever tests it could.
 */
function missingArtifact(note) {
  if (process.env.CI) throw new Error(note.replace(/^note: /, "CI: "));
  console.warn(note);
}

...
(Track B / B2). Builds on B0 (`plans/2026-07-30-trio-tumble-b0-specials.md`, wrapped is
**created** from an L/T match) and B1 (`plans/2026-07-30-trio-tumble-b1-striped.md`, the
...
- Activation lives in `activate` + `resolve_move` (`crates/trio-tumble-core/src/engine.rs`),
...
- `crates/trio-tumble-core/RULES.md` — extend **T1c — Activation** with the wrapped double:
...
- `crates/trio-tumble-core/vectors/README.md` — note the wrapped double in the step-0
  expectations (a matched wrapped clears its ring at step 0; the centre + a re-blast
  land on the next step).
- `TODO/trio-tumble.md` + roadmap Track B — tick B2.
...
**Read/Write-set:** engine.rs, RULES.md, tests, vectors/10–11, games/trio-tumble/*.json.
...
**Changes:**
- [ ] `src/games/trio-tumble*.ts` — the traced animation already steps through the two-blast
...
(how-to + e2e + docs). Final gate: cargo (trio-tumble-core + trio-tumble-solver, incl. 8 new

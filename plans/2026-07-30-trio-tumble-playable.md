...
**Status:** ✅ **SHIPPED (v1) 2026-07-30.** `/trio-tumble/` is playable: an 8×8 / 6-colour
...
specials, drag) tracked in `TODO/trio-tumble.md`. Objective decided with the owner:
...
`trio-tumble-core` is a green **mechanics engine** (swap → find-matches → clear →
...
2. **No starting deal.** Boards are built from char grids (`from_rows`); there is
   ...
   legal move.
3. **No browser binding / UI.** `trio-tumble-wasm` is a stub; nothing renders.
...
Goal: `/trio-tumble/` is a real, verifiable, accessible game meeting every standard in
...
- **M1 — core deal generator.** `trio-tumble-core`: `deal(seed, w, h, colors) ->
...
A stranger opens `/trio-tumble/`, gets the daily 8×8 deal, swaps adjacent gems (legal

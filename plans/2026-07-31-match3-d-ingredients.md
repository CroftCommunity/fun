...
- **`ingredients_remaining`** is the win check (monotone non-increasing — refill makes
  only gems). `Mode::Ingredients` + a `Match3Ingredients` outcome kind
  (`match3-ingredients`), a `board_view` ingredient mask, an ingredients-left HUD.
...
- `crates/match3-core/vectors/*` + README — an ingredient authoring char + vectors.
- `docs/BUILDING-GAMES.md` — note the first *falling non-gem* cell if it generalizes.
- `TODO/match3.md` + roadmap Track D — tick Ingredients.
...
- [ ] `match3-solver` — `find_ingredients` (budgeted DFS, progress = ingredients
  collected, reusing `search`); `generate_ingredients_pack`; committed
  `games/match3/ingredients-pack.json` + regeneration drill (365 winnable, byte-identical).

### Phase 3 (binding) — `Mode::Ingredients` + outcome + board_view
- [ ] `match3-wasm` + `match3-wasm.ts` — `new_ingredients_game`, `Match3Ingredients`
  outcome kind (`match3-ingredients`), `board_view` ingredient mask + remaining/total.
...
- [ ] roadmap + `TODO/match3.md` tick Ingredients.

...
**Changes:**
- [x] `src/games/checkers/checkers-howto.ts` (new) — the guide as pure data,
      covering the two things a player will get wrong: **capture is mandatory** and
      **crowning ends the move**.
- [x] `src/how-to-registry.ts` — register `checkers`.
- [x] `tools/guide-shots.mjs` — the checkers `SHOTS` entries.
- [x] `assets/guide/checkers-*.jpg` — generated by `npm run guide:shots`.
...
**Landed:** `checkers-howto.ts` (six sections), the `how-to-registry` entry, four
`SHOTS` definitions, and `assets/guide/checkers-{board,capture,tutor,result}.jpg`.
RED by construction, exactly as the plan predicted: registering the guide turned
`tests/how-to.test.ts`'s "every screenshot exists" red before a shot existed.

**Shooting the UI found a real CSS bug that no test had.** `.checkers-board` is a
grid with fixed-width columns, so as a block-level element it stretched its brown
felt to the container width. Inside `.checkers-game` (a centred flex column) that
is invisible; on the **result screen**, whose parent is a plain block, the board
rendered as an 8×8 grid marooned in a wide brown rectangle. Fixed with
`width: fit-content; margin-inline: auto`. Every automated check passed while it
was broken — the axe pass, the narrow-viewport pass, and the e2e that asserts
`.checkers-board.checkers-final` is visible all held, because the board *was*
visible and *did* fit. Looking at the picture is what caught it, which is the
argument for this phase existing.

**Two shots came out byte-identical, which was a content bug, not a flake.** The
first `checkers-board` definition played four plies before selecting a man — and
because capture is mandatory, by then the only offered move was a jump, so it
produced the same picture as `checkers-capture` (checkers' start position does not
depend on the seed, and first-legal play is deterministic, so two different seeds
converge). `checkers-board` now shoots the **opening**, picking the man with two
destinations so the caption's "gold dots" is literally true; `checkers-capture`
keeps the mid-game jump. Worth remembering: in a mandatory-capture game you cannot
get a "here is a quiet move" screenshot from a mid-game position.

**The alt text was written before the shot and was wrong.** It described the
result as "The Engine won 0–7"; the real terminal from the first-legal line is the
no-progress draw at 161 moves, "A draw 1–6", with two crowned kings on the board.
Corrected against the picture. An alt text written from imagination is a caption
for a screenshot that does not exist.

**Churn discipline held:** `npm run guide:shots` rewrote all 30-odd JPEGs;
`git checkout --` reverted every non-checkers one and `git status` was checked
before the commit, per `fun/CLAUDE.md`.
...
**Pre-existing and unrelated:** the 11 Node-25 `trio-tumble` failures (see Phase 1

...
**Status:** planning (2026-07-30). Parent: `plans/2026-07-30-match3-parity-roadmap.md`
(Track B / B1). Builds on B0 (`plans/2026-07-30-match3-b0-specials.md`): striped
...
- Creation + the cascade loop live in `resolve_move`
  (`crates/match3-core/src/engine.rs`), which already computes `creations_for`
...
- `crates/match3-core/RULES.md` — a new **T1c — Activation** section (trigger
...
- `crates/match3-core/vectors/README.md` — note activation in the step-0
  expectations (a matched striped clears its whole line at step 0).
- `TODO/match3.md` + roadmap Track B — tick B1.
...
**Read/Write-set:** engine.rs, RULES.md, tests, vectors/08–09, games/match3/*.json.
...
**Changes:**
- [ ] `src/games/match3*.ts` — the traced animation already steps through the

...
`plans/2026-07-30-match3-parity-roadmap.md` (Track B / B0). Core rules:
`crates/match3-core/RULES.md`. Standards: `docs/BUILDING-GAMES.md`.
...
`games/match3/{par-pack,blockers-pack,jelly-pack}.json` no longer match what the
...
- **Pack tests will break without regeneration.** `committed_par_pack_is_wellformed`
  (`crates/match3-solver/tests/solver.rs:278`) asserts
  `e.tiers == par_tiers(e.seed)` for the first 3 entries; `par_tiers`
  (`match3-solver/src/lib.rs:245`) replays via `reference_score`/`_beam`/`random_score`,
...
- **Board authoring/rendering seams exist.** `from_rows_with_jelly`
  (`board.rs:153`), `BoardView.jelly` (`match3-wasm/src/lib.rs:277`),
  jelly rendering in `src/games/match3.ts` — all direct precedents for the
  special overlay's authoring, view, and render.

## Documentation Impact

- `crates/match3-core/RULES.md` — **Phases 1–5.** Board model (special overlay),
...
- `crates/match3-core/vectors/README.md` — **Phase 3.** Document the special
...
- `TODO/match3.md` — **Phase 7.** Tick B0; record the representation decision,
  the 2×2/fish→B4 deferral, and the pack regeneration.
- `plans/2026-07-30-match3-parity-roadmap.md` — **Phase 7.** Update the Track B
...
- [ ] **D1: Exactly which committed data breaks, and does winnability survive?**
  - **Probe:** On a scratch branch (or a throwaway experiment), add minimal
    ...
    `cargo test -p match3-solver` and `cargo test -p match3-core`. Record which
...
**Changes:**
- [ ] `crates/match3-core/src/board.rs` — a `SpecialKind` enum
...
- [ ] `crates/match3-core/src/hash.rs` — append `s\x00` + one tag byte per cell
  **only when** any special is present (jelly pattern).
- [ ] `crates/match3-core/RULES.md` — board model gains the special overlay; the
  P1-scope "Specials in v1: none" line is updated to point here; the state-hash
  spec gains the special section.
- [ ] `crates/match3-core/tests/specials.rs` (new) — gem-only board hash ==
...
2. **Verification:** `cargo test -p match3-core` green (incl. the new
...
**Changes:**
- [ ] `crates/match3-core/src/engine.rs` — `clear_cells` zeros `special_at` on
...
- [ ] `crates/match3-core/RULES.md` — note specials fall/clear with their gem.
- [ ] `crates/match3-core/tests/specials.rs` — a special falls with its gem
...
2. **Verification:** `cargo test -p match3-core` green.
...
**Changes:**
- [ ] `crates/match3-core/src/engine.rs` (or new `src/shapes.rs`) — `find_runs`
...
- [ ] `crates/match3-core/RULES.md` — T1 extension (run structure), the
  shape-priority table, the line-4 rule + orientation, the creation-placement
  tie-break table.
- [ ] `crates/match3-core/vectors/04-striped-from-line4.json` (new, hash locked)
  + `crates/match3-core/tests/specials.rs` / `shapes.rs` cases: swap-formed
...
2. **Verification:** `cargo test -p match3-core` green incl. `04` locked.
...
**Changes:**
- [ ] `crates/match3-core/src/engine.rs` / `shapes.rs` — act on the L/T and
  line-5 classifications; apply the priority order (≥5 > L/T > 4 > 3) so a
  component that qualifies as several yields one special.
- [ ] `crates/match3-core/RULES.md` — finalise the L/T and line-5 rules + the
...
2. **Verification:** `cargo test -p match3-core` green incl. `05`–`07`.
...
**Goal:** Specials render on `/match3/` with an accessible label and are in the
...
- [ ] **5a** `crates/match3-wasm/src/lib.rs` — `BoardView` gains a `specials`
  ...
  test.
- [ ] **5b** `src/games/match3.ts` — render the overlay as a badge/backing on the
  gem tile with a text/aria label (not colour-only — a11y); reduced-motion-safe;
  `src/games/match3-howto.ts` gains a "special candies" note.
...
**Depends on:** Phase 4.
**Read-set:** match3-wasm/src/lib.rs, src/games/match3.ts, match3-howto.ts.
**Write-set:** 5a → match3-wasm/src/lib.rs (+ a wasm test); 5b → match3.ts,
...
- [ ] Run the `#[ignore]` generators (`generate_par_pack_file`,
  ...
  `games/match3/{par-pack,blockers-pack,jelly-pack}.json`.
...
- [ ] Re-embed: the par table is `include_bytes!`-embedded in the wasm
  (`match3-wasm/src/lib.rs:48`); rebuild the wasm so the embedded table matches.
...
(the wasm build).
**Read-set:** match3-solver/src + tests, games/match3/*.json, match3-wasm/src.
**Write-set:** games/match3/par-pack.json, blockers-pack.json, jelly-pack.json
...
**Changes:**
- [ ] `TODO/match3.md` — tick B0; record the overlay decision, 2×2/fish→B4,
  the pack regeneration.
- [ ] `plans/2026-07-30-match3-parity-roadmap.md` — Track B header: B0 done,

...
(`plans/2026-07-30-trio-tumble-playable.md`). Standards: `docs/BUILDING-GAMES.md`.
Backlog source: `TODO/trio-tumble.md` → "Deferred".
...
2. **A mid-run deadlock ends the round** instead of reshuffling. To stay
   ...
   `TrioTumble::replay` reshuffles identically) — which can move locked
...
- **Item 1 (contained):** add an *additive* `Game::play_move_traced` in
  `trio-tumble-core` that does exactly what `play_move` does — same RNG stream, so the
...
- **Item 4 (fuzzy + versioning):** evaluate a shallow lookahead/beam
  ...
  `TrioTumble::VERSION` and keep the old par for old-version records. Ship with a
...
- **D2 (item 4 versioning):** **DECIDED 2026-07-30: build the stronger reference,
  hold the switch.** Add a beam `reference_score_beam` to trio-tumble-core and validate
  ...
  **Adoption procedure (when real data justifies it):** switch `targets_for` to
  the beam reference and/or new fractions, bump `TrioTumble::VERSION` to 2, and keep
  the greedy par for version-1 records (read the version from the `pond-docformat`
  envelope in `TrioTumble::replay`). Treat any par change as a rules-version bump.
- **Doc drift:** `TODO/trio-tumble.md` says fractions are 40/66/100%; the shipped code
  (`trio-tumble-wasm` `targets_for`) is 30/60/90%. Fix the doc during item 4.
...
- [x] **Item 3** — clear-the-blockers, shipped 2026-07-30 as a full slice across
  ...
  `trio-tumble-solver` crate + committed winnable-daily pack, 3c the mode-aware binding
  (`new_blockers_game`, `TrioTumbleBlockers` outcome kind), 3d the UI objective toggle

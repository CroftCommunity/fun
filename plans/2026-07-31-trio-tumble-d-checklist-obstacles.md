...
(Track D — parity completeness). Sibling: `plans/2026-07-31-trio-tumble-d-ingredients.md`
...
- **Reuse the proven mode template** (blockers/jelly/ingredients): core state + rules +
  vectors → a `trio-tumble-solver` finder + committed winnable pack → a mode-aware binding
...
- **The checklist win is path-accumulated, not board-state.** Unlike blockers/jelly/
  ...
  **session/solver state**, fed by a `ChecklistProgress` accumulator that reads each
  `MoveReport`. The accumulator + the seed→targets template live in **`trio-tumble-core`** so
...
- The binding plays moves via `play_swap`/`play_swap_traced` → `game.play_move`, discarding
  ...
  accumulator (only when `Mode::Checklist`). Confirmed by reading `trio-tumble-wasm/src/lib.rs`.
...
- `crates/trio-tumble-core/vectors/*` + README — a checklist vector (colour + specials tally)
...
- `TODO/trio-tumble.md` + roadmap Track D — tick the checklist + obstacle families **and record
  timed / spreading-chocolate / meta as deliberately out of scope, with reasons**.
- `src/games/trio-tumble-howto.ts` — the guide currently says "four objectives"; the checklist
...
- [x] `trio-tumble-solver` — `find_checklist(seed, node_budget)` (a progress-carrying DFS:
  ...
  `trio-tumble-checklist-pack`). Committed `games/trio-tumble/checklist-pack.json` (365 seeds, fixture
...
- [x] `trio-tumble-wasm` — `Mode::Checklist`; `new_checklist_game`; `Session` carries
...
- [x] `trio-tumble-solver` — `find_obstacles` (reuses the board-state `search`, `won` =
  `blockers_remaining==0`, key = blocker layers removed — same as `find_clear`);
  `generate_obstacles_pack`; `obstacles_pack_to_doc` (`trio-tumble-obstacles-pack`). Committed
  `games/trio-tumble/obstacles-pack.json` (365 seeds, fixture seed 72 / 1-move line) + regen drill
...
- [x] `trio-tumble-wasm` — `Mode::Obstacles`; `new_obstacles_game`; `board_view` obstacle mask
  ...
  `TrioTumbleObstacles` outcome (`trio-tumble-obstacles`); `outcome_json` arm. TS wrapper
...
- [x] `TODO/trio-tumble.md` + `plans/2026-07-30-trio-tumble-parity-roadmap.md` — ticked the checklist
...
`TODO/trio-tumble.md` and the roadmap; marked **Track D COMPLETE** and updated the roadmap's
...
free); `TrioTumbleObstacles` outcome (`trio-tumble-obstacles`). TS wrapper + `OBSTACLES_KIND` + verify
...
(`trio-tumble-obstacles-pack`). Committed `obstacles-pack.json` (365 seeds, fixture seed 72 /
...
target colour + the three (progress, target) pairs; `TrioTumbleChecklist` outcome
(`trio-tumble-checklist`, replay re-accumulates progress). TS wrapper (`newChecklistGame` +
...
`generate_checklist_pack` + `checklist_pack_to_doc` (`trio-tumble-checklist-pack`) mirror the
...
`trio_tumble_core::checklist` module holds `checklist_targets(seed, colors)` (seed-derived colour

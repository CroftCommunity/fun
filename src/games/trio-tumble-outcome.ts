//! Pure outcome/verify/share logic for Trio Tumble — no DOM. The verifiable claim:
...
/** A `pond_outcome::Record` for Trio Tumble as it crosses the wasm boundary. */
...
export const BLOCKERS_KIND = "trio-tumble-blockers";
export const JELLY_KIND = "trio-tumble-jelly";
export const INGREDIENTS_KIND = "trio-tumble-ingredients";
export const CHECKLIST_KIND = "trio-tumble-checklist";
export const OBSTACLES_KIND = "trio-tumble-obstacles";

/** The minimal binding surface [`verifyRecord`] drives (the `TrioTumble` wrapper satisfies it). */
...
/** Encode a Trio Tumble outcome envelope as the `?r=` share payload. */
export async function encodeRecord(env: M3Envelope): Promise<string> {
  return encodeShare(env);
}

/** Decode a `?r=` share payload back into a Trio Tumble outcome envelope. */

//! Typed TS wrapper over the `trio-tumble-wasm` raw C-ABI binding. Loads the wasm,
...
/** A loaded Trio Tumble binding bound to one game. */
export class TrioTumble {
  private constructor(private readonly x: Exports) {}

  static async load(wasmUrl = "/trio-tumble.wasm"): Promise<TrioTumble> {
    const source =
    ...
    return new TrioTumble(instance.exports as unknown as Exports);
  }
